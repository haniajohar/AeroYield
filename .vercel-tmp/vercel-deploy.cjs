...
const ALLOWED_COMMANDS = new Set(['vercel', 'npx', 'npm', 'pnpm', 'yarn']);
function log(msg) {
  console.error(msg);
}
...
function doDeploy(projectPath, options) {
  ...
  if (options.name) cmdParts.push('--name', options.name);
  if (options.prod) {
    cmdParts.push('--prod');
  ...
  try {
    ...
    // Also try to match any vercel.app URL in the output
    const anyUrlMatch = output.match(/(https:\/\/[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*\.vercel\.app)/i);
    const fallbackUrl = anyUrlMatch ? anyUrlMatch[1] : null;
    // Use aliased URL (public) first, fallback to deployment URL
    const finalUrl = productionUrl || deploymentUrl || fallbackUrl;
    log('');
    log('========================================');
