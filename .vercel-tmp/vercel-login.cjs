...
const ALLOWED_COMMANDS = new Set(['vercel', 'npx']);
function log(msg) {
console.error(msg);
}
...
// Use npx vercel (local install) as primary, global vercel as fallback
...
log('========================================');
log('');
log('To switch accounts, run: npx vercel logout');
log('');
